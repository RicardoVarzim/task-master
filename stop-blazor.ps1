# Script to stop Task Master Blazor
$originalDirectory = Get-Location

try {
    Write-Host "Stopping Task Master Blazor..." -ForegroundColor Yellow

    # Try to find processes using port 5001
    $port = 5001
    $pidsToStop = @()
    ...
    # Also try to stop dotnet processes that might be running the Blazor
    $dotnetProcesses = Get-Process dotnet -ErrorAction SilentlyContinue
    foreach ($proc in $dotnetProcesses) {
        try {
            $cmdLine = (Get-CimInstance Win32_Process -Filter "ProcessId = $($proc.Id)").CommandLine
            if ($cmdLine -like "*TaskMaster.Blazor*") {
                Write-Host "Stopping dotnet process (PID: $($proc.Id)) running TaskMaster.Blazor" -ForegroundColor Cyan
